  ...
  this.readDirFiles = function() {
    ...
    myEmitter.on('fileRead', (fileName, data, metaData) => {
      ...
      const splitData = data
        ...
        .filter(
          f =>
            f.toLowerCase().search(new RegExp(searchFilter.toLowerCase())) != -1
        )
        .map(d =>
          d
            .split('\n')
      ...
      if (counter === fileCount)
        fileData = Array.from(
          new Set(fileData.map(d => JSON.stringify(d)))
        ).map(d => JSON.parse(d));
    ...
    readDirectory(rootDir)
      .then(files => {
        const fileNames = Array.from(
          new Set(files.filter(f => !fileNameFilter.includes(f)))
        );
        let counter = 0;
        for (const fileName of fileNames) {
          readFile(relativePath + fileName, 'utf8')
            .then(data => {

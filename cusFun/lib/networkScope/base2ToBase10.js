module.exports = function base2ToBase10 (data){
  return new Promise ((res) => {
    const base10 = data.reduce((n,o) => {
      ...
      const wildMaskBase10 = mask.map(d => (255 - parseInt(d,2).toString(10))).join(".");
      ...
      const prefixToBits = mask.reduce((n,o) => {
        const asNumber = o;
        const networkBits = [...asNumber.toString().split("").map((d) => +d).filter((f) => f === 1)];
        const hostBits = [...asNumber.toString().split("").map((d) => +d).filter((f) => f === 0).map((d) => d + 1)];
        n.networkBits.push(... networkBits);
        n.hostBits.push(... hostBits);
        return n;
      }, {networkBits:[],hostBits:[]});
      const bits = Object.keys(prefixToBits).map((d) => ({[d]: prefixToBits[d].reduce((n,o) => n + o ,0)}));
      const {networkBits} = bits[0];
      const {hostBits} = bits[1];
      const range =  Math.pow(2,sudoPrefix) === Infinity ? 1 : Math.pow(2,sudoPrefix);
      
      const subnetIncrement = [128, 64, 32, 16, 8, 4, 2, 1].filter((f,i) => ++i == networkBits%8).pop();
      const subnets = hosts;
      n.push({
        ip: ipBase10,
        mask: maskBase10,

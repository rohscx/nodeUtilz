  ...
  return new Promise(resolve => {
    ...
    const outPut = ipArray.map((d, i) => {
      dns.reverse(d, function(err, domains) {
        if (err != null || domains == undefined)
          return myEmitter.emit('responseError', err ? err : domains);
        domains.forEach(function(domain) {
          dns.lookup(domain, function(err, address, family) {
            const lookupResponse = { [address]: domain };
            myEmitter.emit('response', lookupResponse);
            const dataCount = i + 1;
            if (ipArray.length == dataCount)
              myEmitter.emit('finished', [response, responseError]);
          });
        });
      });
    });

...
module.exports = function (data) {
    ...
    return new Promise((resolve, reject) => {
        ...
        if (typeof(data) !== 'string') rejection(data,`Data type: ${typeof(data)}`, reject);
        ...
        if (!optionType === 'f1') rejection(splitData,'Option Type is not F1', reject);
        // coerce to int
        const optionLength = +splitData[1];
        if (optionLength > 16) rejection(splitData, `Cisco option maximum limit of 4 exeeded. Total found in option: ${optionLength/4}`, reject);
        ...
        if (groupedBase10Values.length !== ipCount) rejection(splitData, `Option declared Addresses: ${ipCount} Decoded Addresses: ${groupedBase10Values.length}`, reject);

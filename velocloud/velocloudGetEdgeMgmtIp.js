  ...
  const optionMaker = (data, url, authCookie) => {
    ...
    return (options = {
      callBack: function(data) {
        ...
        const edgeProfile = data.filter(
          f => f.name === 'Edge Specific Profile'
        )[0];
        const profileDeviceSettings = edgeProfile.modules.filter(
          f => f.name == 'deviceSettings'
        )[0];
  ...
  return Promise.all(
    requestData.map((d, i) => {
      return new Promise((resolve, reject) => {
        setTimeout(() => {
          velocloudGetEdgeConfigurationStack(d)
            .then(t => {

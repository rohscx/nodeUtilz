...
module.exports = function putBoxFile(
  BoxSDK,
  boxSecurityObject,
  boxAppUserId,
  folderId,
  ...
  appUserClient.folders
    ...
    .then(t => {
      const directoryFilesObj = t.entries.map(({ name, id }) => ({
        [name]: id,
      }));
      ...
      if (directoryFilesObj) {
        ...
        if (directoryFileNames.includes(fileName)) {
          myErrorLogs.addLog(
            `\n\n File already exsists in Box folder! FileName: ${fileName} ID: ${getFileId(
              directoryFilesObj,
              fileName
            )} \n\n`
          );
          appUserClient.files
            .uploadNewFileVersion(
              getFileId(directoryFilesObj, fileName),
              stream
            )
            .then(t => {
              myLogs.addLog(
                `Successfully Up Loaded New Version of file: ${fileName} to box:\n`
              );
              myLogs.addLog(t);

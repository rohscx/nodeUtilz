...
module.exports = function boxFolders(BoxSDK, boxSecurityObject, boxAppUserId, folderId, days, callBack) {
  const myLogs = new LogCapture();
  // check for missing options
  [BoxSDK, boxSecurityObject, boxAppUserId, folderId, days, callBack].map((d, i) => {
  ...
  appUserClient.folders.getItems(folderId, {fields: 'name,id,created_at'})
      .then((rootFolder) => {
        ...
        const iseReports = Promise.all(iseExports.map((d, i) => {
          const chunks = [];
          return new Promise(function(res, rej) {
            appUserClient.files.getReadStream(d.id, null, function(error, stream) {

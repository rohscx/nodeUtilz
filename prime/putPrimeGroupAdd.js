...
module.exports = function putPrimeGroupAdd(
  targetGroupName,
  apiAuth,
  filterKey,
  filterValue,
   ...
   * @param {string} targetGroupName Group Name.
   * @param {obj} apiAuth object.
   * @param {string} filterKey filter key.
   * @param {string} filterValue filter string.
   * @param {string} localFilePath local file path.
   * @param {function} callBack callback to send the result.
   * @param {function} debug console.log debugging function.
   * @return {int} The sum of the two numbers.
   */
  async function makePrimeIseSwitchList(
    targetGroupName,
    apiAuth,
    filterKey,
    filterValue,
    ...
    const primeDeviceFilter = getPrimeDeviceType(
      primeData,
      filterKey,
      filterValue
    ...
    const cleanLocalFileData = localFile0
      .replace(regEx0, '')
      .split('\r\n')
      .filter(f => f.length !== 0)
      .map(d => d.split('-'));
    const acaObj = cleanLocalFileData.reduce((n, o) => {
      ...
      if (!n[acaName]) {
        n[acaName] = {
          [branchName]: [deviceNameIp(primeDeviceFilter, filter)],
        };
    ...
    if (debug)
      return debug([
        JSON.stringify(acaObj, null, '\t'),
        targetDeviceIpAddresses,
        targetDeviceIds,
        targetDeviceNames,
      ]);
    const postBody = {
    ...
    const addDevicesToPrimeGroup = getRequestOptions(
      'PUT',
      addUrl,
      addUri,
      addQs,

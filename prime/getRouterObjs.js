  ...
  for (const d of keys) {
    ...
    for (const d1 of keys1) {
      for (const d2 of dataCopy[d][d1]) {
        ...
        const routerData = JSON.parse(request)
        ...
        const {
          ...
          ethernetInterfaces = {
            ethernetInterface: [{
              name: null, network: null, vlan: null, speed: null,
            }],
          },
        } = routerData;
        ...
        const interfaceSpeeds = ethernetInterface.reduce((n, o) => {
          ...
          if (speed!== null) {
            n[name] = typeof(speed.longAmount) == 'number' ? (speed.longAmount/1000000).toString() : 'N/A';
          }
        ...
        for (each of ipInterface) {
          ...
          each.speed = interfaceSpeeds[name] === undefined ? 'N/A' : interfaceSpeeds[name];
        ...
        const deviceData = {
          routerData: summary,
          looBackZero: emptyEmpty
              .filter((f) => ((f.name.toLowerCase().search('loopback') !== -1) && (f.operationalStatus.toLowerCase() != 'down'))),
          interfaceTunnel: emptyEmpty
              .filter((f) => ((f.name.toLowerCase().search('tu') !== -1) && (f.operationalStatus.toLowerCase() != 'down'))),
          interfaceGi: emptyEmpty
              .filter((f) => ((f.name.toLowerCase().search('gi') !== -1) && (f.operationalStatus.toLowerCase() != 'down'))),
          interfaceFe: emptyEmpty
              .filter((f) => ((f.name.toLowerCase().search('fa') !== -1) && (f.operationalStatus.toLowerCase() != 'down'))),
          interfaceMu: emptyEmpty
              .filter((f) => ((f.name.toLowerCase().search('multi') !== -1) && (f.operationalStatus.toLowerCase() != 'down'))),
          cdpNeighbors: cdpNeighbor,
          siteData: siteData,
        };

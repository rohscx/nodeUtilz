...
module.exports = function(voice, video, screenShare) {
  return new Promise((resolve, reject) => {
    ...
    const voiceKbps =
      userVoiceCount > 10
        ? {
            up: userVoiceCount * audioLarge,
            down: userVoiceCount * audioLarge,
          }
        : {
            up: userVoiceCount * audioSmall,
            down: userVoiceCount * audioSmall,
          };
    ...
    const screenSharingUp =
      userScreenShareCount > 0
        ? userScreenShareCount * 2048 + (userScreenShareCount + 800)
        : 0; // 800 kbps for remote control
    const screenSharingDown =
      userScreenShareCount > 0
        ? userScreenShareCount * 2048 + (userScreenShareCount + 800)
        : 0; // 800 kbps for remote control
    ...
    const totalKbps = {
      ...
      down: totals(
        [voiceKbps, videoKbps, videoHdKbps, screenSharingKbps],
        'down'
      ),
    };
    const result = {
      voiceKbps,
      voiceMpbs: asMbps(voiceKbps),
      videoKbps,
      videoMbps: asMbps(videoKbps),
      videoHdKbps,
      videoHdMbps: asMbps(videoHdKbps),
      screenSharingKbps,
      screenSharingMbps: asMbps(screenSharingKbps),
      totalKbps,
      totalMbps: asMbps(totalKbps),
    };
    resolve(result);
  });
};
